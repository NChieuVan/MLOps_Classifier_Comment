...
def load_params(params_path:str)->dict:
    ...
    try:
        ...
        logger.debug("Parameters retriever from %s",params_path)
        return params
...
def load_data(data_url:str)->pd.DataFrame:
    ...
    try:
        ...
        logger.error("Unexpected error occurrd while loading the data: %s",e)
        raise
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # Remove rows with empty strings in 'clean_comment'
        df = df[df['clean_comment'].str.strip() != '']

        logger.debug('Data preprocessing completed: Missing values, duplicates, empty strings removed.')
        return df
...
def save_data(train_data:pd.DataFrame, test_data:pd.DataFrame,data_path:str)->None:
    ...
    try:
        ...
        # Create the data/raw directory if it dose not exsist
...
def main():
    try:
        ...
        logger.debug('Failed to complete the data ingestion: %s',e)
        raise
